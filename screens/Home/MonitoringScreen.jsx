...
import {
  ...
  Modal, PermissionsAndroid, Linking, ScrollView, AppState // ⬅️ AppState để pause/resume poll
} from 'react-native';
...
let CameraRoll;
if (Platform.OS !== 'web') {
  try { CameraRoll = require('@react-native-camera-roll/camera-roll').CameraRoll; } catch {}
}
...
const STICKY_CHARGING_MS = 60_000; // giữ “Đang sạc” nếu vừa ghi nhận charging trong 60s
const ZERO_TO_READY_SEC = 0;       // optional: ép ready nếu kw=0 đủ lâu (0 = tắt)

// Map<device_code, { status?:'online'|'offline', ts:number, ports: Map<number, {portStatus?:string, powerKW?:number, energyKWh?:number, end?:string, zeroSince?:number, lastChargingAt?:number, completed?:boolean, ts:number}> }>
...
function adaptDeviceInfoToUI(info, sessionsDev, lang) {
  ...
  const uiPorts = ports.map((p) => {
    ...
    if (deviceOffline) {
      ...
      visual = 'charging'; textStat = 'charging';      // API busy ⇒ charging
      ...
      const isRun = ['running','in_progress','active','charging'].includes(sessStat); // SSE/session ⇒ charging
...
function applyLiveToUI(ui) {
  ...
  next.ports = (ui.ports || []).map((p) => {
    ...
    const baseIsCharging = (visual === 'charging');

    // live status (SSE hoặc Poll) đẩy lên
    if (fresh && liveP.portStatus) {
      const mapped = mapPortStatusToVisual(liveP.portStatus);
      if (mapped) {
        const mappedIsReady = mapped === 'ready';
        // Không downgrade từ charging → ready nếu chưa completed
        if (!(baseIsCharging && mappedIsReady && !liveP.completed)) {
          visual = mapped; text = mapped;
        }
      }
    }

    // giữ dính “Đang sạc” trong 60s nếu vừa thấy charging
    if (!fresh && baseIsCharging && liveP.lastChargingAt && (Date.now() - liveP.lastChargingAt) < STICKY_CHARGING_MS) {
      visual = 'charging';
      text   = 'charging';
    }
      ...
      <View style={styles.modalContent}>
        <View style={styles.modalInfoCard}>
          {[
            ...
            [t('end'), modalPort.end],
            [t('power'), `${modalPort.kw} W`],
            [t('energy'), `${modalPort.kwh} Wh`],
          ].map(([k, v], i) => (
        ...
        <View style={styles.qrWrap}>
          ...
          <View style={styles.modalActions}>
            <TouchableOpacity style={styles.btnPrimary} onPress={() => openLink(buildCheckoutUrl(modalPort.portNumber))}>
...
/* ===================== SCREEN ===================== */
const POLL_MS = 10_000; // poll 10s

export default function MonitoringScreen() {
  const { t, lang } = useI18n();

  // Spacer cho navBottom fixed (nếu có)
  ...
  const handleOpenPortModal = useCallback((port) => { setModalPort(port || null); setShowModal(Boolean(port)); }, []);
  ...
  // Backfill 1 lần sau khi SSE open (chống miss gói charging đầu)
  let lastBackfillAt = useRef(0);
  const BACKFILL_GAP_MS = 15_000;
  const backfillAfterSseOpenOnce = useCallback(async () => {
    const now = Date.now();
    if (now - lastBackfillAt.current < BACKFILL_GAP_MS) return;
    lastBackfillAt.current = now;

    const devId = selectedId;
    if (!devId) return;
    try {
      const uiBase = await fetchDeviceDetail(devId);
      baseDeviceRef.current = uiBase;
      setSelectedDevice(applyLiveToUI(uiBase));
      saveDeviceUiToCache(devId, uiBase);
    } catch (e) {
      console.log('backfill error', e?.message || e);
    }
  }, [selectedId, fetchDeviceDetail, saveDeviceUiToCache]);

  useEffect(() => {
    const off = sseManager.on((evt) => {
      // Lắng nghe status open để backfill 1 lần
      if (evt?.type === 'status' && evt.status === 'open') {
        backfillAfterSseOpenOnce();
        return;
      }
      ...
      if (pt === 1) {
        ...
        if (Number.isFinite(pn)) {
          ...
          dev.ports.set(pn, {
            ...
            powerKW,
            energyKWh,
            // nếu power > 0 → đang sạc
            portStatus: powerKW > 0 ? 'charging' : (cur.portStatus || undefined),
            lastChargingAt: powerKW > 0 ? now : (cur.lastChargingAt || undefined),
        ...
        if (Number.isFinite(pn)) {
          ...
          const isCompleted = /completed|finish|finished/i.test(String(obj?.portStatus || ''));
          dev.ports.set(pn, {
            ...cur,
            portStatus: mapped,             // flip → ready nếu completed
  ...
  }, [getSelectedDeviceCode, backfillAfterSseOpenOnce]);

  // Tick mỗi 1s: TTL overlay & optional zero→ready
  useEffect(() => {
  ...
  /* ========== POLLING 10s getDevices (chống overlap, dừng khi background) ========== */
  ...
  const startPoll = useCallback(() => {
    ...
    pollTimer.current = setInterval(async () => {
      ...
      try {
        ...
        // Overlay nhanh cho device đang chọn (OR: kw>0 || status busy ⇒ charging)
        if (selectedId) {
          const base = baseDeviceRef.current;
          const found = arr.find(d => (d?._id === selectedId) || (d?.device_code && d.device_code === base?.code));
          if (found && base) {
            const code = found.device_code || base.code;
            const now = Date.now();
            const dev = { ports: new Map(), ts: now };
            dev.status = normalize(found.status) === 'offline' ? 'offline' : 'online';

            const ports = Array.isArray(found.ports) ? found.ports : [];
            for (const p of ports) {
              const pn = Number(p?.portNumber);
              if (!Number.isFinite(pn)) continue;
              const kw = Number(p?.kw) || 0;
              const rawStatus = normalize(p?.status); // idle/busy/...
              // Ưu tiên: power > 0 → charging; không thì map từ status
              let mapped = kw > 0 ? 'charging' : mapPortStatusToVisual(rawStatus) || 'ready';
              const entry = {
                powerKW: kw >= 1000 ? kw / 1000 : kw, // nếu kw là W thì đổi kW; nếu đã là kW thì giữ
                energyKWh: undefined,
                portStatus: mapped,
                lastChargingAt: mapped === 'charging' ? now : undefined,
                zeroSince: mapped !== 'charging' ? now : undefined,
                ts: now,
              };
              dev.ports.set(pn, entry);
            }
          ...
          // Nếu cần kéo detail để có session đầy đủ khi phát hiện đổi trạng thái:
          // await loadDeviceDetail(selectedId, { swr: false });
        }
      } catch (e) {
  ...
  }, [selectedId, saveMenuToCache, t /*, loadDeviceDetail*/]);
  ...
  const saveQrPng = useCallback(async () => {
    ...
    try {
      ...
      if (Platform.OS === 'android' && Platform.Version < 29) {
        ...
        if (granted !== PermissionsAndroid.RESULTS.GRANTED) { toast(t('toastNeedStorage')); return; }
      }
      qrRef.current.toDataURL(async (base64Data) => {
        try {
          ...
          toast(t('toastSaved'));
        } catch (err) { console.error('Save error:', err); toast(err?.message || 'Save failed'); }
      });
    } catch (err) { console.error('saveQrPng native error:', err); toast(err?.message || 'Save failed'); }
  }, [modalPort, selectedDevice, toast, t, buildCheckoutUrl]);
    ...
    const icon =
      selectedDevice?.deviceStatus === 'offline'
        ...
        : (item.visualStatus === 'charging'
            ? plugCharging
            : (item.visualStatus === 'ready' ? plugOnline : plugOffline)); // 'fault' tạm dùng offline
    ...
    return (
      <View style={[styles.portCard, { marginHorizontal: 16 }]}>
        ...
        <View style={styles.cardActions}>
          <TouchableOpacity style={styles.btnDetail} onPress={() => handleOpenPortModal(item)} activeOpacity={0.85}>
...
const styles = StyleSheet.create({
  ...
  btnDetailText: { color: '#2563EB', fontWeight: '800' },
  ...
  modalOverlayAbs: {
    ...
    padding: 20, zIndex: 2147483647, justifyContent: 'center', alignItems: 'center',
  },
  ...
  modalHeader: {
    ...
    paddingHorizontal: 20, paddingTop: 20, paddingBottom: 16, borderBottomWidth: 1, borderBottomColor: '#E5E7EB',
  },
  ...
  btnGhost: {
    ...
    paddingVertical: 10, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', flex: 1,
  },
