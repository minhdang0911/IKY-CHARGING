// components/overview/KPIStats.jsx
...
export default function KPIStats({ L, revenueData }) {
  ...
  return (
    <View style={[s.row, narrow && { flexWrap: 'wrap', gap: 8 }]}>
      <View style={[s.item, narrow && { flexBasis: '48%' }]}>
        <Text style={s.label}>{L.yoy}</Text>
        <View style={s.valRow}>
          <Icon
            name={momUp ? 'trending-up' : momDown ? 'trending-down' : 'horizontal-rule'}
            size={16}
            color={momUp ? '#16A34A' : momDown ? '#EF4444' : '#64748B'}
          />
          <Text
            style={[
              s.value,
              momUp && { color: '#16A34A' },
              momDown && { color: '#EF4444' },
            ]}
          >
            {mom}%
          </Text>
      ...
      <View style={[s.item, narrow && { flexBasis: '48%' }]}>
        <Text style={s.label}>{L.ytd}</Text>
        <Text style={s.value}>{fmtMoney(ytd)}</Text>
      </View>

      <View style={[s.item, narrow && { flexBasis: '48%' }]}>
        <Text style={s.label}>{L.avg}</Text>
        <Text style={s.value}>{fmtMoney(avg)}</Text>
      </View>

      <View style={[s.item, narrow && { flexBasis: '48%' }]}>
        <Text style={s.label}>{L.activeMonths}</Text>
        <Text style={s.value}>{activeMonths}</Text>
      </View>
    </View>
  );
}
